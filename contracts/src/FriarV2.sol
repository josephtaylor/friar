...
///
/// 2. `filterPeriod` IS PER-POOL CONFIGURATION, not a hook-wide constant. LB fixes it at
///    deploy time, so serving pools with different flow density means deploying different
///    hooks. Here it is chosen per pool and frozen at initialization.
///
///    AN EARLIER VERSION MADE THIS WINDOW ADAPTIVE, tracking an EWMA of inter-swap gaps.
///    It was cut, and the reasoning is worth keeping because it is the same reasoning that
///    should gate any future attempt:
///
///    - Cadence is not economically meaningful. A $0.01 swap and a $100k swap are one
///      observation each, so on a cheap chain the signal is manipulable by dust.
///    - Normalising it economically collapses it. The natural normaliser is notional over
///      active liquidity, which IS price displacement, which is what the LB accumulator
///      already measures. The separate cadence channel turns out not to measure anything
///      independent.
///    - Routing is endogenous. Adaptive charges more, charging more on a thin pool costs
///      routing, and estimated loop gain was 4.5x stronger in the thin regime (0.264) than
///      the routed one (0.059) — strongest exactly where a bootstrapping pool lives. Its
///      apparent advantage was measured under an exogenous-flow assumption that production
///      violates.
///    - Sparse observation is a SYMPTOM. A pool sees long gaps because it loses routes;
///      making it charge its few remaining trades more attacks the cause backwards.
///
///    The failure directions are also asymmetric, which is why a per-pool constant defaults
///    to LB's value rather than to something matched: too SHORT merely means the surge never
///    fires (the pool behaves like a static-fee pool), while too LONG means references stop
///    refreshing, the accumulator ratchets to its ceiling, and the fee pins high enough to
///    starve routing. Regime drift runs thin -> routed, i.e. INTO the severe direction, so
///    the safe default is the short end.
///
...
contract FriarV2 is IHooks {
    ...
    /// @dev LB's own `filterPeriod`. The default, and the conservative end: too short only
    /// costs surge revenue, too long starves routing.
    uint16 public constant DEFAULT_FILTER_PERIOD = 10;
    ...
    struct PoolConfig {
        ...
        uint16 filterPeriod; // LB filterPeriod, per pool instead of per hook
    ...
    function _validate(PoolConfig memory c) internal pure {
        if (
            c.baseFeePips == 0 || c.baseFeePips > MAX_BASE_FEE_PIPS || c.filterPeriod == 0
                || c.decayPeriod > MAX_DECAY_PERIOD || c.filterPeriod > c.decayPeriod
                || c.reductionFactor > FriarMath.BASIS_POINT_MAX || c.maxVolatilityTicks == 0
                || c.maxVolatilityTicks > MAX_VOLATILITY_TICKS
    ...
    function previewFee(PoolKey calldata key) external view returns (uint24) {
        ...
        FriarMath.Params memory p = _lbParams(c, c.filterPeriod, _binStep(key.tickSpacing));
    ...
    function _accrueAndQuote(PoolKey calldata key) internal returns (uint24) {
        ...
        PoolConfig memory c = configOf(poolId);
        FriarMath.Params memory p = _lbParams(c, c.filterPeriod, binStep);
    ...
    function effectiveMaxVolatilityAccumulator(PoolKey calldata key) external view returns (uint24) {
        return
            _lbParams(configOf(key.toId()), DEFAULT_FILTER_PERIOD, _binStep(key.tickSpacing)).maxVolatilityAccumulator;
    }
